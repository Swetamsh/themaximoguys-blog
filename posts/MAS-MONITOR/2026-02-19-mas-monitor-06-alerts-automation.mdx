...
> **MAS 9.2 stream outlook -- edge-local anomaly processing (announced for the 9.2 stream, per Naviam partner outlook -- not confirmed by IBM release notes):** Edge-local anomaly processing is on the announced roadmap for the 9.2 continuous-delivery stream, aimed at reducing alert latency in geographically distributed deployments by scoring anomalies on the edge gateway rather than round-tripping every reading to the cloud. Treat this as the trajectory the stream is taking, not a shipped capability -- continue designing alert latency budgets around cloud-side scoring for now.
