...
## MAS 9.2 Stream: AI Service 9.2.0 and the gpt-oss-120b Model Swap

The most concrete change for Monitor users joining the MAS 9.2 continuous-delivery stream sits in the **AI Service component**. AI Service is the central LLM gateway for every MAS AI feature -- it's the layer that Watsonx Assistant, FMEA reasoning, and the Predictive/Maintenance Copilot features (PCC/MCC) all call into. **AI Service 9.2.0 was released January 29, 2026**, and the headline change is the default model swap: `openai/gpt-oss-120b` (an open-source 120-billion-parameter model from OpenAI) now replaces the prior default `ibm/granite-3-2-8b-instruct`, which IBM deprecated in February 2026.

For Monitor specifically, this gateway powers two day-to-day workflows: **Watsonx Assistant** for natural-language anomaly investigation Q&A ("why is Pump-031's vibration trending up this week?") and **FMEA reasoning** for failure-mode-and-effects analysis grounded in the asset's live sensor history. The model swap brings substantially more parameters and reasoning capacity to those flows. If you've built Monitor dashboards or alert workflows that lean on Watsonx Assistant explanations, expect the responses to be richer once your environment is on AI Service 9.2.0 -- and re-test any prompt-shaped expectations you've baked into runbooks. The change applies to MAS 9.2 environments and is also backported to 9.1.11+.

*Outlook (announced for the 9.2 stream, per Naviam partner outlook -- not confirmed by IBM release notes):* AI-powered **anomaly enrichment** that automatically tags asset records in Manage when Monitor detects an anomaly is on the announced roadmap for the 9.2 stream. Treat this as the trajectory, not as a shipped capability you can architect around today.
