...
## What's New in the MAS 9.2 Stream for Monitor

MAS 9.2 is not a single sealed GA release. It is a **continuous-delivery Feature Channel stream** -- IBM ships components on a monthly readme cadence (verified Sep 2025 through Mar 2026), and the platform carries `9.2.0-pre.stable_<build>` tags rather than a traditional version number. If you are planning a Monitor upgrade, treat 9.2 as a *stream you join* rather than a *version you install*.

Here is what that stream brings to Monitor users:

- **AI Service component 9.2.0 (released January 29, 2026)** -- The central LLM gateway for MAS AI features. `openai/gpt-oss-120b` is now the default model, replacing `ibm/granite-3-2-8b-instruct` (deprecated February 2026). Monitor's Watsonx Assistant for anomaly investigation and FMEA reasoning rides on this gateway.
- **Platform stack** -- OpenShift 4.16-4.21, MongoDB 7.0/8.0 (Community + Enterprise), Cloud Pak for Data 5.2.0. If you are still on OCP 4.12 or Mongo 6.x, plan the platform refresh before joining the 9.2 stream.
- **MAS 8.10 and 8.11 EOL** -- Standard support and software updates end **April 30, 2026**. The runway to move forward is short.
- *Announced for the 9.2 stream (per Naviam partner outlook -- not confirmed by IBM release notes):* lightweight IoT ingestion options for use cases that don't need full Monitor compute, AI-powered anomaly enrichment that auto-tags asset records when anomalies are detected, and edge-local anomaly processing to reduce alert latency in distributed deployments.

We flag the outlook items deliberately. Until they appear in IBM's official release notes or component-level GA announcements, treat them as the trajectory the stream is taking, not as shipped features you can architect around today.
