...
## Planning for the MAS 9.2 Continuous-Delivery Stream

If you are scoping a Monitor upgrade or a multi-quarter implementation, build your governance around the reality that **MAS 9.2 is a continuous-delivery Feature Channel stream, not a sealed GA release**. IBM has not issued a 9.2 GA announcement letter; components carry `9.2.0-pre.stable_<build>` tags on a monthly readme cadence (verified Sep 2025 through Mar 2026). That changes how you plan.

**Platform stack to standardize on for the 9.2 stream:**

- **OpenShift 4.16-4.21** supported
- **MongoDB 7.0 and 8.0** (Community + Enterprise)
- **Cloud Pak for Data 5.2.0**
- AppPoints licensing model unchanged from 9.0/9.1

If your current environment is on OCP 4.12 or Mongo 6.x, the platform refresh is the gating prerequisite before you join the 9.2 stream.

**EOL clock you cannot ignore:** **MAS 8.10 and 8.11 standard support and software updates end April 30, 2026.** If you are still on 8.x, the runway is short -- treat the upgrade path planning as a 2026 priority, not a 2027 backlog item.

**Upgrade governance considerations -- continuous delivery changes the playbook:**

1. **Plan for monthly Feature Channel cadence, not a single GA cutover.** Build a regression test pack you can run against each monthly readme rather than gating everything on a one-time go-live.
2. **Pin to a known-good build for production change windows.** Catalog versions like `v9-260326-amd64` are concrete reference points -- use them when negotiating change windows with ops.
3. **Track AI Service separately.** AI Service 9.2.0 (Jan 29, 2026 component-level GA) carries the `gpt-oss-120b` model swap that affects Watsonx Assistant outputs. Validate prompts and AI-flavored runbooks each time AI Service rolls forward.
4. **Treat Naviam/partner outlook items as roadmap, not commitments.** Lightweight IoT ingestion, AI-powered anomaly enrichment, and edge-local anomaly processing have been *announced for the 9.2 stream per partner outlook -- not confirmed by IBM release notes*. Architect for them as options, not as shipped features.
