...
> **MAS 9.2 stream outlook -- lightweight IoT ingestion (announced for the 9.2 stream, per Naviam partner outlook -- not confirmed by IBM release notes):** The 9.2 Feature Channel is expected to introduce lightweight ingestion options for use cases that don't need full Monitor compute -- think small device fleets, infrequent telemetry, or compliance-only data collection where the full analytics pipeline is overkill. If your roadmap includes thousands of low-signal sensors that today would be uneconomical on standard Monitor pricing, watch this space. Until it lands in IBM release notes, design your current ingestion around the MQTT/HTTP patterns above and revisit when the option is GA.
