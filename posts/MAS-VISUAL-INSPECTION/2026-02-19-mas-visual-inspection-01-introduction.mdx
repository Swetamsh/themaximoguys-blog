...
## MVI on the MAS 9.2 Stream

Before we go further: a quick orientation on where MVI sits in IBM's continuous-delivery cadence as of May 2026.

MVI is currently shipping on the **MAS 9.2 Feature Channel** -- a continuous-delivery feature stream, not a sealed GA release. The IBM CLI catalog `v9-260326-amd64` (March 26, 2026) lists the MVI component at version **`9.2.0-pre.stable_12598`**, with monthly catalog drops carrying incremented build numbers. Treat 9.2 as the next step in the existing 9.x trajectory, not a re-platforming.

**What that means for MVI specifically:**

- Every 9.0 feature we reference in this series -- Kepler GPU removal, GigE Vision camera support, Data Lifecycle Manager, facial redaction, v2 MQTT APIs to Monitor, GPU workload optimization, edge diagnostics dashboards -- is still in force on the 9.2 stream. Nothing has been rolled back.
- Every 9.1 change still applies: SSD training is deprecated, existing SSD models continue to run inference, ITSM workflow support is available.
- The one fully confirmed 9.2 component change relevant to MVI is the **AI Service 9.2.0** release on January 29, 2026, which replaced the `ibm/granite-3-2-8b-instruct` model with `openai/gpt-oss-120b` for PCC, MCC, and FMEA workflows. Granite was deprecated in February 2026. We cover this in detail in Part 9 (Integration) and Part 10 (Governance).
- **Announced for the 9.2 stream (per Naviam partner outlook -- not yet confirmed by IBM release notes):** geofencing/geospatial visual inspection workflows, 3D asset views for inspection context, and AI-powered video inspection enhancements. Where we mention these (notably Parts 8, 9, and 10), we flag them as outlook items.

The series is current. The version numbers, GPU support matrix, deprecation timelines, and platform stack details all reflect the 9.2 stream as observed through the March 2026 catalog drop. We will keep these posts updated as IBM publishes additional 9.2 release notes.
