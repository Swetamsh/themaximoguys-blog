...
### What is MAS 9.2 and how does it affect MVI?

**Short answer:** MAS 9.2 is a continuous-delivery Feature Channel stream, not a sealed GA release. As of the March 26, 2026 IBM CLI catalog drop (`v9-260326-amd64`), the MVI component ships at **`9.2.0-pre.stable_12598`**, with monthly catalog refreshes incrementing the build number. Treat 9.2 as the next step in the existing 9.x trajectory rather than a re-platforming.

**Diagnosis:** If you are reading IBM partner content or third-party blogs that reference "MAS 9.2 features for MVI," verify each item against the IBM release notes and the actual catalog build before planning around it. Some 9.2 items (e.g., AI Service 9.2.0 with the gpt-oss-120b model swap) are IBM-documented and shipped. Others (e.g., geofencing/geospatial workflows, 3D asset views, AI-powered video inspection enhancements) are flagged in partner outlooks as announced for the 9.2 stream but are not yet confirmed by IBM release notes.

**What still applies from 9.0 and 9.1:** Every consideration in this migration section continues to be relevant on the 9.2 stream. Kepler GPUs are still unsupported. SSD training is still deprecated. v2 MQTT APIs are still the integration path to Monitor. GPU workload optimization, GigE Vision camera support, Data Lifecycle Manager, facial redaction, edge diagnostics dashboards, and ITSM workflow support all carry forward into 9.2 unchanged. Do not remove these items from your upgrade checklist when moving to the 9.2 stream.

**The one fully confirmed 9.2-stream change affecting MVI workflows:** AI Service 9.2.0 (released January 29, 2026) replaces `ibm/granite-3-2-8b-instruct` with `openai/gpt-oss-120b` for PCC, MCC, and FMEA reasoning. For MVI, this affects the AI-generated narrative descriptions and FMEA outputs that ride on top of inspection findings flowing into Manage. Granite was deprecated February 2026.

**Action:** Pin and document the exact MVI catalog build (`9.2.0-pre.stable_<build>`) in your governance records. On a continuous-delivery platform, the catalog build is the only reliable way to reproduce model behavior. Also note: **MAS 8.10 and 8.11 standard support and software updates end April 30, 2026** -- any site still on those releases in May 2026 is operating outside the supported window for MVI.
