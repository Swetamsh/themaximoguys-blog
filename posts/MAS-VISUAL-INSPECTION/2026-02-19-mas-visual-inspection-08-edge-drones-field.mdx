...
### Outlook: Geofencing and Geospatial Visual Inspection (9.2 Stream)

```
  GEOFENCING FOR VISUAL INSPECTION
  =================================
  ...
  WHAT IT IS:
  ──────────
  Geofencing/geospatial workflows tie MVI
  inspections to physical location boundaries:

  - Trigger an inspection when a drone enters
    a defined geofence (e.g., transmission corridor,
    substation perimeter, pipeline right-of-way)
  - Auto-tag every captured image with its
    geofence membership for later analytics
  - Route alerts based on geofence ownership
    (e.g., "Defect in District 4 substation →
     District 4 supervisor's queue")
  - Skip inspections when assets are out of
    the approved flight zone (compliance + safety)

  HOW IT FITS THE EDGE PATTERN:
  ────────────────────────────
  Geofence membership becomes another input to
  the MQTT alert rule engine. The same rule that
  fires "if crack detected + confidence > 85%"
  can be extended to "AND inside geofence X."

  This is especially relevant for:
  - Drone fleets covering large utility corridors
  - Multi-site mining or industrial campuses
  - Pipeline inspection where ROW boundaries
    determine ownership and response
  - Offshore platforms with regulated airspace

  STATUS: Tracking the IBM release notes for the
  9.2 stream. As of the March 26, 2026 catalog drop
  this remains a partner-outlook item, not a
  shipped capability documented by IBM. Where you
  see geofencing referenced for MVI today,
  validate against the actual MVI catalog build
  (current: 9.2.0-pre.stable_12598).
```

> **Action for architects:** If geofencing is on your roadmap, design your edge alert rule schema today so it can carry a geofence-membership field. That way, when the capability lands in the 9.2 stream, your existing rules extend rather than rewrite.
