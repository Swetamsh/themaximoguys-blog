...
## The MAS 9.2 Platform Stack (Where MVI Lives Today)

If you are planning infrastructure in 2026, build to the 9.2 Feature Channel stack -- not the 8.x stack you may have used previously. MVI ships on the same platform substrate as the rest of MAS.

```
  MAS 9.2 SUPPORTED PLATFORM STACK
  =================================

  OpenShift:        4.16 - 4.21 supported
  MongoDB:          7.0 and 8.0 (Community + Enterprise)
  Cloud Pak for
  Data:             5.2.0
  Licensing:        AppPoints (unchanged from 9.0/9.1)

  CRITICAL EOL DATE:
  ──────────────────
  MAS 8.10 and 8.11 standard support and software
  updates END APRIL 30, 2026.

  If you are still running 8.10 or 8.11, you have
  weeks, not quarters, to plan your upgrade path
  before falling off support.

  MVI CATALOG VERSION (Mar 26, 2026 snapshot):
  ──────────────────────────────────────────
  MVI: 9.2.0-pre.stable_12598
  (Continuous-delivery Feature Channel build —
   not a sealed GA release. Catalog drops monthly.)
```

> **Key insight:** Procurement teams often size infrastructure to "the GA release we will adopt." On 9.2 there is no single GA release -- there is a continuous-delivery stream. Plan for monthly catalog refreshes, not annual upgrades. Your OpenShift, MongoDB, and Cloud Pak for Data versions need to track inside the supported windows above.
