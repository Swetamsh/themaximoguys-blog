...
### MAS 9.2 Continuous-Delivery Governance

```
  GOVERNING THE 9.2 FEATURE CHANNEL
  ===================================

  9.2 is NOT a sealed GA release.
  It is a continuous-delivery / Feature Channel
  stream with monthly catalog drops.
  Current MVI build (Mar 26, 2026):
    9.2.0-pre.stable_12598

  GOVERNANCE IMPLICATIONS:
  ───────────────────────
  1. Plan rolling, not annual, upgrades.
     Build a monthly cadence into your change
     advisory board. Treat each catalog drop as
     a minor upgrade — regression-test models,
     edge devices, and integrations against the
     new build before promoting to production.

  2. Pin and document the catalog build in use.
     Record `9.2.0-pre.stable_<build>` in your
     model cards and audit logs so you can tie
     a model's behavior to a specific platform
     version after the fact.

  3. AI Service 9.2 model-swap governance.
     The Jan 29, 2026 swap of
     `ibm/granite-3-2-8b-instruct` →
     `openai/gpt-oss-120b` for PCC/MCC/FMEA is
     the most material 9.2 change touching MVI
     outputs. Update model cards to record
     gpt-oss-120b as the new narrative-reasoning
     source. Granite was deprecated Feb 2026.

  4. 3D asset views (announced for 9.2 stream,
     per partner outlook — NOT yet IBM-confirmed)
     may affect data retention policy:
     - Multi-angle / 3D image volumes can be
       5-10x the storage of 2D inspection sets
     - Revisit DLM retention windows BEFORE the
       capability lands, so storage planning is
       not retroactive
     - Flag this as a roadmap item, not a
       shipped fact, in any governance doc that
       references it

  5. MAS 8.10 / 8.11 EOL — April 30, 2026
     ───────────────────────────────────────
     Standard support and software updates end.
     Any site still running 8.10 or 8.11 in
     May 2026 is operating an unsupported
     platform for MVI. Governance bodies should
     treat this as a P1 audit finding.
```

> **Action for the governance lead:** Add a "9.2 stream catalog build" field to every model card you maintain. The build number is the only reliable way to reproduce model behavior on a continuous-delivery platform.
