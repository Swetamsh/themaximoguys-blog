...
### Beyond Health: The Five Score Types

Health is the most visible score, but MAS Health actually calculates **five distinct score types** for each asset:

| Score Type | What It Tells You | How It Is Calculated |
|------------|------------------|---------------------|
| **Health** | Current condition and likelihood of failure | Weighted combination of condition indicators across components |
| **Criticality** | Consequence of failure across business dimensions | Safety, production, regulatory, and financial impact ratings |
| **Risk** | Priority for action (Health x Criticality) | Matrix combination of health and criticality scores |
| **End of Life** | When the asset will likely need replacement | Age, condition trajectory, and design life calculations |
| **Effective Age** | How old the asset "acts" based on its condition | An asset in poor health has an effective age greater than its actual age; a well-maintained asset may have an effective age lower than its calendar age |

End of Life and Effective Age are particularly powerful for capital planning. A 10-year-old pump that has been run hard and poorly maintained might have an effective age of 18 years -- meaning its health profile matches what you would expect from an 18-year-old asset. That distinction changes capital conversations.

IBM provides **built-in sample contributors** to get you started quickly:

- `FACOPERATINGSAMPLE` -- operating status (running, standby, decommissioned)
- `FACAGESAMPLE` -- asset age relative to expected life
- `FACMTBFSAMPLE` -- mean time between failures
- `FACYTDBGTCOSTSAMPLE` -- year-to-date budget cost percentage
- `FACRULYEARSSAMPLE` -- remaining useful life in years
- `FACYTDREPCOSTSAMPLE` -- year-to-date repair costs

These samples are starting points. Every production deployment should customize contributors per asset class based on your available data and failure modes. But they are an excellent way to see your first scores within days rather than weeks.
