...
## 📊 The Four Asset Views in MAS Health

MAS Health provides four distinct views for analyzing your asset fleet. Each serves a different analytical purpose:

| View | What It Shows | Best For |
|------|-------------|----------|
| **Table** | Sortable list with health, criticality, risk, end of life, and effective age columns | Prioritized work planning, finding the worst assets fast |
| **Map** | Geographic view with color-coded pins for health status; container polygons show aggregate health for sites or areas | Multi-site organizations, facilities management, field crew dispatch |
| **Charts** | Health wheel categorizations, unplanned downtime by month, failure rates by manufacturer, MTBF metrics | Trend analysis, identifying systemic issues, fleet-level patterns |
| **Matrix** | Two-variable scatter plot (e.g., Criticality vs. End of Life) with assets plotted as points | Risk distribution analysis, identifying clusters of high-risk assets, capital planning |

The **Matrix view** is particularly powerful and often underutilized. By plotting assets across two condition variables, you can instantly spot dangerous clusters -- like a group of high-criticality assets all approaching end of life simultaneously. That pattern is invisible in a table but obvious in a matrix.

The **Map view** uses color-coded pins (green for healthy, yellow for watch, red for critical) and supports container boundaries that show polygon outlines with aggregate health scores for each site or zone. For organizations with geographically dispersed assets, the map view is often the executive's preferred entry point.

> **Practitioner tip:** Start stakeholder meetings with the Map or Matrix view to set context, then drill into the Table view for action items. Charts work best for monthly reliability reviews where you need trend data.
