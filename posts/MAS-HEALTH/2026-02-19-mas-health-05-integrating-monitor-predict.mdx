...
> **AI Service 9.2 -- what changed under the hood (Jan 29, 2026):** The MAS 9.2 stream introduces **AI Service component 9.2.0**, which swaps the underlying LLM from `ibm/granite-3-2-8b-instruct` to `openai/gpt-oss-120b` for Predictive Condition Classification (PCC), Maintenance Component Classification (MCC), and FMEA reasoning workflows. The previous Granite model is deprecated as of February 2026. Any Health features that consume AI Service -- asset Q&A, recommended actions, FMEA-driven indicator generation -- inherit the upgrade automatically; no Health model reconfiguration is required. This is currently the most concrete 9.2-specific change visible in Health.

### Dissolved Gas Analysis (DGA) Widgets for Transformers

MAS Health includes **dedicated DGA widgets** for transformer health monitoring. Dissolved Gas Analysis -- tracking gas concentrations like hydrogen, methane, ethylene, and acetylene in transformer oil -- is one of the most reliable early failure indicators for power transformers.

The DGA widgets in Health visualize gas concentration trends over time, flag abnormal gas ratios, and feed directly into transformer health scoring. This means your transformer fleet health scores can incorporate DGA results alongside traditional indicators like loading, temperature, and age -- giving you a multi-dimensional view of transformer condition that catches incipient faults months before they become failures.

> **Why this matters:** DGA has been an industry-standard diagnostic for decades, but the results often lived in separate lab databases or spreadsheets. Health brings DGA data into the same scoring and risk framework as every other asset, so a transformer with concerning gas trends gets the risk priority it deserves.

### Condition Monitoring Integration

When connecting IoT sensor data to Health, MAS provides a **"Use Condition Monitoring" checkbox** on meter-based scoring factors. Enabling this feature:

- **Aligns thresholds** -- sets High/Low threshold values calibrated to your IoT sensor reading ranges
- **Enables real-time updates** -- health indicators update when new sensor readings arrive, not just on manual inspection cycles
- **Bridges Monitor and Health** -- sensor data flows from Monitor through to Health scoring without manual data entry

This is the technical mechanism that makes the difference between "snapshot" health (updated monthly) and "heartbeat" health (updated continuously). For assets with IoT instrumentation, enabling Condition Monitoring transforms Health from a periodic reporting tool into a live operations dashboard.
