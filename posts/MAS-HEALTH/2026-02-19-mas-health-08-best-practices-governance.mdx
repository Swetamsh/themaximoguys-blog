...
draft: false
tier: "developer"
author: "ACHANTA VENKATA SATYANARAYANA"
...
### What MAS 9.x Already Delivers

With MAS 9.0 (released mid-2025) and MAS 9.1, IBM aligned version numbers across all suite components -- Manage, Health, Predict, and Monitor all share a unified version number. This is more than cosmetic: it reflects tighter integration across the suite.

Key MAS 9.x improvements for Health include:

- **Unified version lifecycle** -- MAS 9 follows a 3+1+3 support structure with monthly maintenance updates (Health patch 9.0.80 is the latest as of early 2026)
- **Enhanced scoring architecture** -- the Scoring Groups → Drivers → Scoring Factors hierarchy provides more granular control over how scores are composed (covered in detail in Part 4)
- **Expanded AI-driven predictive maintenance** -- deeper integration between Predict and Health for real-time scoring updates
- **Four asset views** (Table, Map, Charts, Matrix) for flexible portfolio analysis (covered in Part 6)
- **Pre-configured Work Queues** that surface action-ready asset lists out of the box (covered in Part 7)
- **DGA widgets** for transformer health monitoring using dissolved gas analysis data
- **Condition Monitoring integration** -- the "Use Condition Monitoring" checkbox on meter-based scoring factors bridges IoT data directly into health scoring

### What Is Coming Next

MAS 9.2 has been running as a continuous-delivery Feature Channel since late 2025, with monthly readmes through 2026. No GA announcement letter has been published yet, but production-ready components like AI Service 9.2.0 (Jan 2026) are already shipping under the 9.2 stream -- meaning some 9.2 capabilities are usable today by customers tracking the Feature Channel, while others continue to land readme-by-readme.

> **Plan accordingly:** Standard support and software updates for MAS 8.10 and 8.11 end **April 30, 2026**. If you are still on 8.x, upgrade planning to MAS 9.x should be on your governance calendar now -- not deferred until the 9.2 GA letter arrives.

Looking further ahead, expect:
...
The organizations that build strong Health foundations on MAS 9.x today will be first to adopt these capabilities as they arrive.
