...
## 📋 Pre-Configured Work Queues

MAS Health ships with **pre-configured work queues** that surface the most common action-ready asset lists without requiring custom configuration:

| Work Queue | What It Shows | Action Implication |
|-----------|-------------|-------------------|
| **Assets failing before planned maintenance** | Assets whose health is deteriorating faster than the PM schedule can address | Accelerate PM, add interim inspection, or create corrective work order |
| **Assets with poor health** | All assets currently in the POOR or CRITICAL health bands | Review for intervention -- repair, replace, or increase monitoring |
| **Assets missing data** | Assets where key scoring contributors lack recent data (no meter readings, overdue inspections) | Schedule data collection before scores become stale and misleading |

These queues are available out of the box in MAS 9.x and serve as a practical starting point for Stage 1 (Visibility) of the automation maturity model below. Instead of building custom reports or queries, planners can review these queues during weekly planning meetings to identify high-priority work.

> **Practitioner tip:** The "assets missing data" queue is the one most organizations ignore and should not. A pump with no meter readings in 6 months has a health score based on stale data -- that score is not wrong, it is *fiction*. The missing data queue is your data quality early warning system.
