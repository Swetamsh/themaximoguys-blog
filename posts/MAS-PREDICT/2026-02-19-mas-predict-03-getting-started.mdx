...
> **Platform stack on the MAS 9.2 stream (as of early 2026):** If you are landing Predict on the 9.2 continuous-delivery stream, the supported platform underneath is OpenShift 4.16 through 4.21, MongoDB 7.0 or 8.0 (Community or Enterprise), and Cloud Pak for Data 5.2 for the data and model-training plane. AppPoint-based licensing is unchanged. Keep in mind two timing facts: **the Predict component itself is still on the 9.0.x cadence (latest `9.0.12`, Feb 26, 2026)** even when the core MAS Platform underneath is on a `9.2.0-pre.stable_*` build, and **MAS 8.10 / 8.11 standard support and software updates end April 30, 2026** -- if you are still on those, plan the upgrade path before deploying new Predict workloads.
