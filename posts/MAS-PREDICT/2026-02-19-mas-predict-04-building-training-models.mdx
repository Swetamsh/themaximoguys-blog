...
> **AI Service 9.2 and the FMEA reasoning layer:** When Predict's classification output feeds into FMEA (Failure Mode and Effects Analysis) narratives and ranked failure-mode suggestions inside MAS, the heavy lifting is now done by the AI Service component. On **AI Service 9.2.0 (released Jan 29, 2026)**, IBM swapped the underlying model for PCC, MCC, and FMEA from `ibm/granite-3-2-8b-instruct` over to **`openai/gpt-oss-120b`** -- a much larger reasoning model that produces noticeably better failure-mode narratives and more useful ranking suggestions when Predict scores are dropped into FMEA context. The Granite model was deprecated in **February 2026**, so any FMEA reasoning you see in MAS 9.2 (or in the 9.1.11+ backport) is gpt-oss-120b under the hood. This is the citable, IBM-documented 9.2-era change that affects Predict consumers directly, even though the Predict component itself remains on the 9.0.x cadence.
