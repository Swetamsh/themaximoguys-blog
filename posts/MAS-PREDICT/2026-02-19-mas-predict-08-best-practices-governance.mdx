...
> **A 9.2-era governance note on upgrade cadence:** MAS 9.2 is a continuous-delivery Feature Channel stream -- monthly readmes, monthly build numbers (e.g. `9.2.0-pre.stable_4447` -> `_9887` -> `_12926` -> `_16717+`), and no single GA cutover date. Plan for rolling upgrades and a change-control process that can absorb a new build every few weeks, not a single big-bang migration. **The Predict component is on its own clock**: latest is `9.0.12` (Feb 26, 2026), still on the 9.0.x cadence -- so your Predict upgrade calendar will diverge from your core MAS Platform / Manage / AI Service upgrade calendar, and your model-inventory governance needs to track both versions per model. Final timing item to bake into your roadmap: **MAS 8.10 and 8.11 standard support and software updates end April 30, 2026.** If you are still running predictive models on an 8.x foundation, the upgrade to a 9.x platform is no longer an "if" -- it is a sequenced project with a hard deadline.
