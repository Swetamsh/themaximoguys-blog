...
### The namespace shift: a compile-time fix
...
### Reflection: the runtime landmine
...
### Removed APIs: the second runtime trap
...
## 🛠️ How to Test and Remediate — the Eight-Step Plan
...
## 🏛️ Why the Model Endured While Everything Else Modernized
...
- **MAS 9.1 (June 2025) moved the JVM from Java 11 to Java 17.** The extension model is unchanged, but the language and runtime around your code are not.
- **The highest-impact change is the `javax.*` → `jakarta.*` namespace shift** — any code that touches Java EE APIs directly must update its imports before it will compile.
- **JPMS module restrictions block reflective access** that worked silently on Java 8/11, and removed deprecated APIs surface as runtime `ClassNotFoundException` or `NoSuchMethodException`.
- **IBM has already updated all PLUS extension code for Java 17** — your remediation scope is custom Java MBOs, MIF exit classes, field validation classes, bean classes, and any bundled third-party libraries.
- **The migration sequence is repeatable:** inventory custom Java, compile against Java 17, fix namespace and deprecated-API errors, test under load, and repackage into a Java-17 Customization Archive.
...
| **Next:** | [Part 7 — Extension Crossovers: PLUSV Collisions, Nuclear's Anomaly & When Products Share DNA](/blog/mas-java-extensions-extension-crossovers) |
