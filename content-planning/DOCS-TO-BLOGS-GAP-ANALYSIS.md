...
| Index only, Parts 1-6 + all covers pending | 1 series | MAS-WATSONX-DATA (DOC13) |
...
| DOC13 | watsonx.data Open Lakehouse (IBM-native counterpart to DOC5) | 🟡 **INDEX ONLY, ASSET BLOCKED** | `posts/MAS-WATSONX-DATA` (series index/Part 0 built 2026-07-19, draft, no cover; Parts 1-6 not yet built, briefs staged in `queue.json`) | Rotate the leaked nanobanana API key (confirmed still `403 PERMISSION_DENIED` as of 2026-07-19, same blocker as DOC5 Parts 5-6), then generate the index cover; write Parts 1-6 |
