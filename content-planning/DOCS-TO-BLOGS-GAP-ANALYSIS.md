...
| DOC5 | Data Analytics & Databricks | 🟡 **IN PROGRESS** | `posts/MAS-DATABRICKS` (index + Part 1 + Part 2 built 2026-07-17 with covers; Parts 3-6 pending) | Build Parts 3-6 (queued in `automation/off-hours/queue.json`), then generate remaining covers |
...
|---:|---|---|---|---|
| 1 | `MAS-DATABRICKS` | DOC5 | **In progress 2026-07-17 (night-shift):** series index (Part 0), Part 1 ("Why Your Maximo Data Belongs in a Lakehouse"), and Part 2 ("Getting Maximo Data Out: Kafka, Data Export, and CDC Patterns") drafted, each with cover. Parts 3-6 not yet written. | Continue via the queued items in `automation/off-hours/queue.json` (databricks-03 through databricks-06). |
